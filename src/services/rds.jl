...
- `"PreSignedUrl"`: The URL that contains a Signature Version 4 signed request for the
  ...
  DB snapshot to copy. You must specify this parameter when you copy an encrypted DB snapshot
...
- `"EngineMode"`: The DB engine mode of the DB cluster, either provisioned, serverless,
  ...
  Aurora User Guide:     Limitations of Aurora Serverless v1      Limitations of Parallel
  Query      Limitations of Aurora Global Databases      Limitations of Multi-Master Clusters
     Valid for: Aurora DB clusters only
...
- `"PreSignedUrl"`: The URL that contains a Signature Version 4 signed request for the
  ...
  must contain the following parameter values:    DestinationRegion - The Amazon Web Services
  Region that the encrypted read replica is created in. This Amazon Web Services Region is
  the same one where the CreateDBInstanceReadReplica action is called that contains this
  ...
  then you call the CreateDBInstanceReadReplica action in the us-east-1 Amazon Web Services
  ...
  CreateDBInstanceReadReplica action that is called in the destination Amazon Web Services
  Region, and the action contained in the presigned URL.    SourceDBInstanceIdentifier - The
  DB instance identifier for the encrypted DB instance to be replicated. This identifier must
  be in the Amazon Resource Name (ARN) format for the source Amazon Web Services Region. For
  example, if you are creating an encrypted read replica from a DB instance in the us-west-2
  Amazon Web Services Region, then your SourceDBInstanceIdentifier looks like the following
  example: arn:aws:rds:us-west-2:123456789012:instance:mysql-instance1-20161115.   To learn
  how to generate a Signature Version 4 signed request, see Authenticating Requests: Using
  Query Parameters (Amazon Web Services Signature Version 4) and Signature Version 4 Signing
  Process.  If you are using an Amazon Web Services SDK tool or the CLI, you can specify
  SourceRegion (or --source-region for the CLI) instead of specifying PreSignedUrl manually.
...
Groups option of the Amazon RDS console or the DescribeDBClusterParameters action to verify
that your DB cluster parameter group has been created or modified. If the modified DB
cluster parameter group is used by an Aurora Serverless v1 cluster, Aurora applies the
...
- `"MasterUserPassword"`: The new password for the master user. The password can include
  ...
  API actions never return the password, so this action provides a way to regain access to a
