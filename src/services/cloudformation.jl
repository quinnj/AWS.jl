...
- `schema_handler_package`: A URL to the S3 bucket containing the extension project package
  ...
  access the package in the S3 bucket. That's, the user needs to have GetObject permissions
...
- `type_name`: The name of the extension being registered. We recommend that extension
  ...
  are reserved and can't be used in your extension names:    Alexa     AMZN     Amazon
  AWS     Custom     Dev
